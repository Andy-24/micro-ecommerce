...
def product_detail_view(request, handle=None):
    ...
    is_owner = False
    if request.user.is_authenticated:
        is_owner = request.user.purchase_set.all().filter(product=obj, completed=True).exists()   #True #verify ownershoip
...
def product_attachment_download_view(request, handle=None, pk=None):
    ...
    if request.user.is_authenticated and can_download is False:
        can_download = request.user.purchase_set.all().filter(product=attachment.product, completed=True).exists() #check ownership
